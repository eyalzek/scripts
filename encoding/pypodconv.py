...
class AVEncoder:
   ...
   def getvParams(self):
      if self.vcodec == 'h264':
         return "-v 1 -vcodec h264 -b %s -bt 175 -refs 2 -loop 1 -deblockalpha 0 -deblockbeta 0 -parti4x4 1 -partp8x8 1 -partb8x8 1 -brdo 1 -me_range 21 -chroma 1 -max_b_frames 0 -level 30 -g 300 -keyint_min 30 -sc_threshold 40 -rc_eq 'blurCplx^(1-qComp)' -qcomp 0.7 -qmax 51 -max_qdiff 4 -i_quant_factor 0.71428572 -b_quant_factor 0.76923078 -rc_max_rate 1500 -rc_buffer_size 244 -cmp 1 -s %sx%s" % (self.vbitrate, self.resolution.w, self.resolution.h)
      elif self.vcodec == 'xvid':
         return "-vcodec xvid -maxrate 2500 -b %s -qmin 3 -qmax 5 -bufsize 4096 -g 300 -s %sx%s" % (self.vbitrate, self.resolution.w, self.resolution.h)
      elif self.vcodec == 'mpeg4':
         return "-vcodec mpeg4 -maxrate 2500 -b %s -qmin 3 -qmax 5 -bufsize 4096 -g 300 -s %sx%s" % (self.vbitrate, self.resolution.w, self.resolution.h)
      else:
         raise UnsupportedCodecError, "Unsupported video codec "+self.vcodec
   def getaParams(self):
      if self.acodec == "aac":
         return "-acodec aac -ab %s -ar 48000 -ac 2" % self.abitrate
      else:
   ...
   def getCommands(self):
      ...
      cmds+=['rm -f .tmp.new.mp4', 
            'MP4Box -add .tmp.mp4 .tmp.new.mp4',
            'mv .tmp.new.mp4 "%s"' % self.output_file,
            'rm -f .tmp.mp4', 'rm -f .tmp.new.mp4']
      return cmds
